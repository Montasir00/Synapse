...
export default function Tasks({ 
  ...
  return (
   <div className="w-full max-w-6xl mx-auto py-6 sm:py-8 lg:py-12 px-3 sm:px-4 lg:px-6 space-y-6 sm:space-y-8 lg:space-y-12">
      ...
      {/* 1. Control Header */}
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 sm:gap-6 lg:gap-8 bg-surface p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[32px] border border-border shadow-sm relative overflow-hidden group">
         <div className="space-y-2 relative z-10">
            ...
            <p className="text-muted text-xs font-medium uppercase tracking-[0.12em] sm:tracking-[0.2em] flex items-center gap-2">
               ...
               {isFocusMode ? 'Focus Protocol Active' : 'Daily command center'}
            </p>
         </div>
         ...
         <div className="flex flex-wrap items-center gap-2 sm:gap-3 lg:gap-4 w-full lg:w-auto relative z-10">
            <div className="relative flex-1 lg:w-64">
               ...
               <input 
                  ...
                  className="w-full min-w-0 bg-surface-subtle py-2.5 sm:py-4 pl-10 sm:pl-14 pr-4 sm:pr-6 rounded-2xl border border-border outline-none text-sm focus:border-accent/40 transition-all"
               />
            </div>

            <button 
               onClick={() => setIsFocusMode(!isFocusMode)}
               className={`flex items-center gap-2 px-4 sm:px-6 py-2.5 sm:py-4 rounded-xl border font-black text-[10px] sm:text-[11px] uppercase tracking-widest transition-all active:scale-95 ${
                  isFocusMode 
            ...
            <button onClick={onAddTask} className="precise-button !px-6 sm:!px-10 !py-2.5 sm:!py-4 shadow-accent/10 w-full sm:w-auto text-[10px] sm:text-sm">
               Add Task
            </button>
         </div>
         {isFocusMode && (
      ...
      <div className={`grid gap-3 sm:gap-6 lg:gap-8 items-start transition-all duration-500 ${
         ...
         <div className="space-y-4 sm:space-y-6 lg:space-y-8">
            <ModuleCard 
               ...
               badge={isFocusMode ? "Priority Focus" : "Primary"}
            >
               {dailyTasks.map(task => (
                   <div key={task.id} className={`flex items-center justify-between p-3 sm:p-4 bg-surface border rounded-2xl group transition-all ${
                     isFocusMode ? 'border-accent/30 shadow-md scale-[1.02]' : 'border-border hover:border-accent/20'
                   }`}>
