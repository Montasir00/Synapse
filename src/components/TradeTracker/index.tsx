    ...
    <div className="w-full max-w-6xl mx-auto space-y-8 sm:space-y-10 pb-20 sm:pb-24 lg:pb-32 px-4 sm:px-6 pt-6 sm:pt-8 lg:pt-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6 border-b border-border/50 pb-6">
         <div>
          ...
          <p className="text-xs sm:text-xs font-black text-muted/60 uppercase tracking-[0.08em] sm:tracking-[0.2em] mt-2">Track open positions, closed trades, and live valuation</p>
         </div>
        ...
        <div className="flex w-full sm:w-auto flex-wrap items-start gap-2 sm:gap-3 mt-2 sm:mt-0">
            ...
            <button 
               ...
               disabled={isSyncing}
            className={`h-10 px-4 sm:px-8 flex items-center gap-2 rounded-full text-xs font-black uppercase tracking-[0.1em] sm:tracking-widest transition-all border ${isSyncing ? 'opacity-50 border-teal-500/10 text-teal-500 bg-transparent' : 'border-teal-500/30 text-teal-500 bg-teal-500/5 hover:bg-teal-500/10'}`}
            >
        ...
        {isLoading ? (
          <div className="rounded-2xl border border-teal-500/20 bg-teal-500/5 px-4 py-3 text-center">
           <p className="text-xs sm:text-xs font-black uppercase tracking-[0.08em] sm:tracking-[0.2em] text-teal-500">Fetching latest trade data</p>
          </div>
        ) : null}
              ...
              {balances
                ...
                .map((b) => (
                 <div key={b.asset} className="flex items-center gap-2 px-2.5 py-1 bg-bg/50 rounded-md border border-border">
                    <span className="text-xs font-bold text-muted uppercase">{b.asset}</span>
                    <span className="text-xs sm:text-xs font-mono font-black text-ink">${Number(b.usdValue || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USDC</span>
                 </div>
              ))}
              {balances.filter((b) => (b.usdValue || 0) > 1).length === 0 && (
                <div className="rounded-md border border-dashed border-border px-3 py-2 text-xs font-black uppercase tracking-[0.2em] text-muted/60">
        ...
        <div className="lg:col-span-7 flex flex-wrap sm:flex-nowrap divide-y sm:divide-y-0 divide-x-0 sm:divide-x divide-border/30 border border-border/50 rounded-2xl bg-surface-subtle/10 shadow-sm">
          {[
            ...
            <div key={i} className="flex-1 w-1/2 sm:w-auto p-4 lg:p-5 flex flex-col justify-center items-center sm:items-start text-center sm:text-left hover:bg-surface/40 transition-colors border-border/30">
              <span className="text-xs font-bold text-muted/50 uppercase tracking-[0.2em] mb-1.5">{m.label}</span>
              <div className="flex items-baseline gap-2">
                 <span className={`text-lg sm:text-xl lg:text-2xl font-mono font-black tracking-tighter ${m.color}`}>{m.value}</span>
                 {m.sub ? <span className="text-xs font-bold text-muted/40 uppercase hidden sm:inline-block">{m.sub}</span> : null}
              </div>
            </div>
          ))}
        </div>
            ...
            <div className="flex w-full sm:w-auto gap-1.5 p-1 bg-surface-subtle/50 rounded-full border border-border">
              {['ALL', 'OPEN', 'CLOSED'].map(f => (
                <button 
                  key={f}
                  onClick={() => setFilter(f)}
                  className={`flex-1 sm:flex-none px-3 sm:px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-[0.1em] sm:tracking-[0.15em] transition-all ${filter === f ? 'bg-accent text-white shadow-lg shadow-accent/20' : 'text-muted hover:text-ink'}`}
          ...
          <div className="space-y-6">
            {filteredPositions.length === 0 ? (
              <div className="glass-card p-16 text-center border-dashed border-border/30">
                ...
                <p className="text-xs sm:text-xs font-medium uppercase tracking-[0.08em] sm:tracking-[0.18em] text-muted/40 max-w-sm mx-auto">
                ...
                >
                    ...
                    <div className="flex items-start gap-4 sm:gap-6 min-w-0 w-full md:w-auto">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 sm:gap-3 mb-2 flex-wrap">
                          ...
                          <span className={`text-xs font-black px-2.5 py-1 rounded-full uppercase tracking-widest border flex items-center justify-center ${pos.status === 'OPEN' ? 'bg-accent/10 border-accent/30 text-accent shadow-[0_0_10px_rgba(114,137,253,0.2)]' : 'bg-surface-subtle border-border text-muted'}`}>
                            {pos.status}
                          </span>
                        </div>
                        <div className="text-xs font-mono font-bold text-muted/60 flex items-center gap-2.5 uppercase tracking-[0.1em] sm:tracking-widest flex-wrap">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 md:gap-12 w-full md:w-auto mt-2 md:mt-0 items-start">
                      <div className="text-left">
                        <span className="block micro-label text-muted/50 mb-2 uppercase tracking-[0.2em] text-xs">AVG_IN</span>
                      ...
                      {pos.status === 'CLOSED' ? (
                        <div className="text-left md:text-center">
                          <span className="block micro-label text-muted/50 mb-2 uppercase tracking-[0.2em] text-xs">AVG_OUT</span>
                        ...
                        <div className="text-left md:text-center">
                          <span className="block micro-label text-muted/50 mb-2 uppercase tracking-[0.2em] text-xs">CURRENT</span>
                  ...
                  {journals[pos.id]?.tags?.length > 0 && (
                    <div className="px-5 sm:px-6 lg:px-8 pb-5 sm:pb-6 lg:pb-8 flex flex-wrap gap-2">
                      {journals[pos.id].tags.map(tag => (
                        <span key={tag} className="px-3 py-1 bg-surface-subtle/50 border border-border rounded-full text-xs font-black text-muted uppercase tracking-widest transition-all hover:border-accent/40 hover:text-accent">
                          {tag}
                        </span>
                      ))}
                    </div>
            ...
            {positions.length > 0 && filteredPositions.length > 0 && filteredPositions.length < positions.length && (
              <div className="text-center text-xs font-black uppercase tracking-[0.2em] text-muted/50 pt-2">
                        ...
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="glass-card !p-3 shadow-xl border-white/10">
                              <p className="text-xs font-black text-muted uppercase tracking-[0.2em] mb-1">Equity Point</p>
            ...
            <div className="space-y-6">
              {biasMetrics.map(mistake => (
                <div key={mistake.tag} className="flex justify-between items-start sm:items-center gap-3 group">
                  <div className="min-w-0">
                    <span className="text-xs font-black text-ink uppercase tracking-[0.1em] sm:tracking-widest truncate block">{mistake.tag}</span>
                    <span className="text-xs font-mono font-bold text-muted/50 uppercase block mt-1">FREQ: {mistake.count}</span>
                  </div>
              ...
              {(Object.values(journals || {}) as JournalEntry[]).every(n => n.tags.length === 0) && (
                <div className="text-center py-8">
                  <p className="text-xs sm:text-xs font-bold text-muted/40 uppercase tracking-[0.08em] sm:tracking-widest mb-3">No symbol tags yet.</p>
                  <p className="text-xs sm:text-xs font-medium text-muted/30 uppercase tracking-[0.06em] sm:tracking-normal leading-relaxed max-w-[220px] mx-auto">Add cognitive tags to your closed positions to unlock bias detection analytics.</p>
                </div>
              )}
            </div>
          </div>
            ...
            >
              <div className="p-4 sm:p-8 border-b border-border flex justify-between items-center gap-3">
                <div>
                  ...
                  <p className="text-xs font-mono font-bold text-accent uppercase tracking-[0.1em] sm:tracking-[0.2em] mt-1 truncate max-w-[230px] sm:max-w-full">{selectedPosition?.symbol} // REF_{selectedPosition?.id.split('_').pop()?.slice(0, 8)}</p>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-8 sm:space-y-10 custom-scrollbar">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {['DISCIPLINED', 'FOMO', 'REVENGE', 'UNCERTAIN'].map(tag => (
                      <button 
                        ...
                        className={`py-3 rounded-lg text-xs font-black uppercase tracking-widest border transition-all ${editingNote.emotionTag === tag ? 'bg-accent border-accent text-white shadow-lg shadow-accent/20 scale-[1.02]' : 'bg-surface-subtle border-border text-muted hover:border-accent/40 hover:text-ink'}`}
                      >
                        {tag}
                      </button>
                    ))}
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                      <label htmlFor="trade-note-planned-stop" className="text-xs font-bold text-muted uppercase mb-2 block">Planned Stop</label>
                      <input 
                    ...
                    <div>
                      <label htmlFor="trade-note-planned-target" className="text-xs font-bold text-muted uppercase mb-2 block">Planned Target</label>
                      <input 
                ...
                <div className="flex items-start sm:items-center justify-between gap-3 p-4 bg-accent/5 border border-accent/20 rounded-xl">
                  <span className="text-xs font-black text-accent uppercase tracking-[0.1em] sm:tracking-widest leading-snug">Followed Pre-Trade Plan</span>
                  <button 
              ...
              <div className="p-4 sm:p-8 border-t border-border bg-surface-subtle/50 flex gap-3 sm:gap-4">
                <button 
                  onClick={() => setIsNoteModalOpen(false)}
                  className="flex-1 py-3 sm:py-4 text-xs font-black text-muted hover:text-ink uppercase tracking-[0.1em] sm:tracking-widest transition-all"
                >
                  Discard
                </button>
                <button 
                  onClick={handleSaveNote}
                  className="flex-[2] bg-accent text-white py-3 sm:py-4 rounded-lg font-black text-xs uppercase tracking-[0.1em] sm:tracking-[0.2em] shadow-xl shadow-accent/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 sm:gap-3"
                >
